...
class FeedAcceptanceTests: XCTestCase {
    ...
    func test_onFeedImageSelection_displaysComments() {
        ...
        XCTAssertEqual(comments.commentsMessage(at: 0), makeCommentMessage())
    }
    ...
    private class HTTPClientStub: HTTPClient {
        ...
        func get(from url: URL, completion:
                 @escaping (HTTPClient.Result) -> Void) -> HTTPClientTask {
            completion(stub(url))
            return Task()
        }
    ...
    private class InMemoryFeedStore: FeedStore, FeedImageDataStore {
        ...
        func insert(_ feed: [LocalFeedImage], timestamp: Date, completion: @escaping InsertionCompletion) {
            feedCache = CachedFeed(feed, timestamp)
            completion(.success(()))
        }
    ...
    private func response(with url: URL) -> (Data, HTTPURLResponse) {
